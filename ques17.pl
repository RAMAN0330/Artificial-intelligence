...
removelast([H|T]):- reverse([H|T],[_H1|T1]),reverse(T1,[_X|Y]),write("Updated List is :- "),write(Y).
...
ques17:-
    write('Enter a list:- '),read([H|T]),nl,
    write('Enter the element to delete: '),read(N),nl,
         delete_nth(N,[H|T]).
